...
public class Enemy : MonoBehaviour
{
    ...
    {
        if (!dead && Player.playerSingleton.CheckAlive())
        {
    ...
    {
        GetComponent<SpriteRenderer>().color = new Color(c.r, c.g, c.b, GetComponent<SpriteRenderer>().color.a);
    }
    ...
    protected void Die(float dmg, int powerType, Vector3 directionHit)
    {
        ...
        //rb2d.AddTorque(Random.Range(dmg * directionHit.x * 5000, dmg * directionHit.x * 15000));
        canMove = false;

        if (enemyNumber != 3)
        {
            CheckDeathType(powerType);
            StartCoroutine(SpinOut(dmg, directionHit));
        }
        else
        {
    ...
    IEnumerator SpinOut(float dmg, Vector3 directionHit)
    {
        float time = 0;
        while (time < 1)
        {
            rb2d.AddTorque(Random.Range(dmg * -directionHit.x * 50, dmg * -directionHit.x * 150));
            time += Time.deltaTime;
            yield return null;
        }
    }
